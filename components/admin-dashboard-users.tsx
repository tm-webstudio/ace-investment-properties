...
import { Users, Mail, Phone, Building, Calendar, Search } from "lucide-react"
...
export function AdminDashboardUsers() {
  ...
  const fetchLandlords = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/users?userType=landlord`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to fetch landlords')
      }
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground min-h-[320px] flex flex-col items-center justify-center">
          ...
          <p className="text-base font-medium mb-1.5 text-red-600">Error Loading Landlords</p>
        ...
        <div className="text-center py-16 text-muted-foreground min-h-[320px] flex flex-col items-center justify-center">
          ...
          <p className="text-base font-medium mb-1.5">{users.length === 0 ? "No Landlords" : "No Matching Landlords"}</p>
          <p className="text-sm max-w-[200px] mx-auto">{users.length === 0 ? "No landlord accounts found" : "Try adjusting your search"}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
