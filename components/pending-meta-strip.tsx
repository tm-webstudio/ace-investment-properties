...
export function PendingMetaStrip({ landlordName, submittedDate, source, onClick }: PendingMetaStripProps) {
  ...
  const content = (
    <div className="flex items-center justify-between gap-3 text-[12px] border border-border/50 bg-white text-foreground px-3 py-2 rounded-none transition-colors hover:border-accent/30 hover:bg-accent/5">
      <div className={`flex items-center ${source === 'facebook' ? 'gap-1' : 'gap-2'} font-medium text-foreground`}>
        <Users className={`h-3.5 w-3.5 ${iconColor}`} />
        {source === 'facebook' && (
          <Facebook className="h-3 w-3 fill-[#1877F2] stroke-none" />
        )}
