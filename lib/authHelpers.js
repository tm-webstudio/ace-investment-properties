...
export async function signUpWithEmail({
  ...
  try {
    ...
    try {
      ...
      console.log('[AUTH] Signup link generation data:', JSON.stringify(linkData, null, 2));
      ...
      if (!linkError && linkData?.properties?.action_link) {
        ...
        if (linkData.properties.action_link.includes('undefined')) {
          console.warn('[AUTH] action_link contains undefined, constructing manual URL');
          // Construct URL manually using hashed_token
          const token = linkData.properties.hashed_token;
          const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
          confirmationUrl = `${supabaseUrl}/auth/v1/verify?token=${token}&type=signup&redirect_to=${encodeURIComponent(siteUrl + '/auth/callback')}`;
        } else {
          confirmationUrl = linkData.properties.action_link;
        }
        console.log('[AUTH] Generated confirmation link:', confirmationUrl);
      } else {
        console.warn('[AUTH] Failed to generate confirmation link, using fallback:', linkError);
      }
    } catch (linkGenError) {
      console.error('[AUTH] Error generating confirmation link:', linkGenError);
    }
    ...
    // Step 4: Send welcome email using our template
    try {
