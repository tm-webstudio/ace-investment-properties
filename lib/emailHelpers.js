...
/**
 * Format property address for email (full address with city and outward postcode)
 * e.g. "123 Nash Road, London, E1"
 * @param {object} property - Property object with address, city, postcode fields
 * @returns {string} Formatted address
 */
export function formatPropertyAddress(property) {
  if (!property) return '';

  const address = property.address || property.street_address || '';
  const city = property.city || '';
  const postcode = property.postcode || '';
  const outwardPostcode = postcode ? postcode.trim().split(' ')[0].toUpperCase() : '';

  const cleanAddr = stripTrailingSuffixes(address, city, outwardPostcode);
  const parts = [cleanAddr, city, outwardPostcode].filter(Boolean);
  return parts.join(', ');
}

/**
 * Format property title for email card display (road name, city, outward postcode)
 * Strips street numbers to match the main site PropertyCard format.
 * e.g. "Nash Road, London, E1"
 * @param {object} property - Property object with address, city, postcode fields
 * @returns {string} Formatted title
 */
export function formatPropertyTitle(property) {
  if (!property) return '';

  let roadName = property.address || property.street_address || '';
  roadName = roadName.replace(/^\d+\s*/, '');
  roadName = roadName.replace(/^(Flat|Apartment|Unit|Suite)\s+\d+[A-Z]?\s*,?\s*/i, '');
  roadName = roadName.replace(/,\s*(Flat|Apartment|Unit|Suite)\s+.*$/i, '');
  ...
  const parts = [roadName, city, outwardPostcode].filter(Boolean);
  return parts.join(', ');
}

/**
 * Strip trailing city and/or outward postcode segments from an address
 * to avoid duplication like "Road, City, PC, City, PC".
 */
function stripTrailingSuffixes(addr, city, outwardPostcode) {
  if (!addr) return addr;
