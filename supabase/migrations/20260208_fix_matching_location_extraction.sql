-- Migration: Fix location extraction in matching functions
-- Date: 2026-02-08
-- Purpose: Handle both localAuthority (singular) and localAuthorities (plural) data structures
...
DECLARE
    ...
    parking_val BOOLEAN;
BEGIN
    -- Extract local authorities (handle both singular and plural)
    SELECT ARRAY(
    ...
    SELECT
        ...
        (
            ...
            CASE
                WHEN p.asking_price BETWEEN min_budget_val AND max_budget_val THEN 25
                ELSE 0
            END
            +
            -- Bedrooms match (20 points)
            CASE
                WHEN cardinality(bedrooms_val) = 0 OR p.bedrooms = ANY(bedrooms_val) THEN 20
                ELSE 0
            END
            +
            -- License match (15 points)
            CASE
                WHEN cardinality(licenses_val) = 0 OR LOWER(TRIM(p.license_type)) = ANY(licenses_val) THEN 15
                ELSE 0
            END
            +
            ...
            CASE
                WHEN NOT parking_val OR p.has_parking = parking_val THEN 5
                ELSE 0
            END
        ...
        ARRAY(
            SELECT reason FROM (
                ...
                WHERE p.asking_price BETWEEN min_budget_val AND max_budget_val
                UNION ALL
                SELECT 'Bedrooms match', 3
                WHERE cardinality(bedrooms_val) = 0 OR p.bedrooms = ANY(bedrooms_val)
                UNION ALL
                SELECT 'License match', 4
                WHERE cardinality(licenses_val) = 0 OR LOWER(TRIM(p.license_type)) = ANY(licenses_val)
                UNION ALL
                ...
                WHERE NOT parking_val OR p.has_parking = parking_val
            ) reasons
            ORDER BY priority
        ) AS match_reasons
    FROM properties p
    ...
    HAVING (
        ...
        END
        +
        CASE WHEN p.asking_price BETWEEN min_budget_val AND max_budget_val THEN 25 ELSE 0 END
        +
        CASE WHEN cardinality(bedrooms_val) = 0 OR p.bedrooms = ANY(bedrooms_val) THEN 20 ELSE 0 END
        +
        CASE WHEN cardinality(licenses_val) = 0 OR LOWER(TRIM(p.license_type)) = ANY(licenses_val) THEN 15 ELSE 0 END
        +
        ...
        +
        CASE WHEN NOT parking_val OR p.has_parking = parking_val THEN 5 ELSE 0 END
    ) >= min_match_score
...
BEGIN
    ...
    SELECT
        ...
        LOWER(TRIM(prop.license_type)),
        LOWER(TRIM(prop.availability)),
        COALESCE(prop.has_parking, FALSE)
    INTO
...
'Matches properties to investor preferences. Handles both localAuthority (singular) and localAuthorities (plural) data structures. Minimum match score is 50 (location match required).';

COMMENT ON FUNCTION find_matching_investors_for_property IS
'Finds matching investors for a property. Handles both localAuthority (singular) and localAuthorities (plural) data structures. Minimum match score is 50 (location match required).';
