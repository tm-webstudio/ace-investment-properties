-- Migration: Fix matching functions with correct data structure
-- Date: 2026-02-08
-- Purpose: Handle actual preference_data structure from frontend
...
BEGIN
    ...
    SELECT
        ...
        ARRAY(
            SELECT reason FROM (
                ...
                WHERE p.bedrooms::INTEGER BETWEEN min_bedrooms_val AND max_bedrooms_val
                UNION ALL
                ...
                WHERE (immediate_availability AND LOWER(TRIM(p.availability)) = 'vacant')
    ...
    HAVING (
        ...
        +
        CASE WHEN p.bedrooms::INTEGER BETWEEN min_bedrooms_val AND max_bedrooms_val THEN 20 ELSE 0 END
        +
        CASE
            WHEN cardinality(property_types_val) = 0
              OR LOWER(TRIM(p.property_type)) = ANY(property_types_val)
