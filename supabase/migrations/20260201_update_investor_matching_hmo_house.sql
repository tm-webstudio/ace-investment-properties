-- Update function to make HMO and House property types interchangeable for investor matching
...
BEGIN
    ...
    -- Expand property types to include HMO/House equivalence
    property_types_expanded := property_types;

    -- Add HMO/House equivalence
    IF 'hmo' = ANY(SELECT LOWER(unnest(property_types))) OR 'hmos' = ANY(SELECT LOWER(unnest(property_types))) THEN
        property_types_expanded := property_types_expanded || ARRAY['house', 'houses', 'hmo', 'hmos'];
    END IF;

    IF 'house' = ANY(SELECT LOWER(unnest(property_types))) OR 'houses' = ANY(SELECT LOWER(unnest(property_types))) THEN
        property_types_expanded := property_types_expanded || ARRAY['hmo', 'hmos', 'house', 'houses'];
    END IF;
    ...
    SELECT
        ...
        (
            ...
            -- Property type match (15 points) - use expanded types including HMO/House equivalence
            CASE
                WHEN array_length(property_types_expanded, 1) IS NULL OR array_length(property_types_expanded, 1) = 0 THEN 15
                WHEN LOWER(p.property_type) = ANY(SELECT LOWER(unnest(property_types_expanded))) THEN 15
                ELSE 0
            END +
        ...
        ARRAY(
            SELECT reason FROM (
                ...
                WHERE LOWER(p.property_type) = ANY(SELECT LOWER(unnest(property_types_expanded)))
    ...
    WHERE p.status::text = 'active'
        ...
        AND (
            ...
            OR
            -- Or property type match - use expanded types
            (array_length(property_types_expanded, 1) IS NULL OR LOWER(p.property_type) = ANY(SELECT LOWER(unnest(property_types_expanded))))
            OR
            -- Or location match
